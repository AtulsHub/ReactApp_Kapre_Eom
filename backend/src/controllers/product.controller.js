...
const editProduct = async (req, res) => {
  try {
    ...
    // If images are being updated, handle file uploads
    if (req.files && req.files.length > 0) {
      let productImages = [];
      for (const file of req.files) {
        const uploaded = await uploadOnCloudinary(file.path);
...
export { createProduct, removeProduct, getProductsByCategory, filterProducts, getProductsPaginated, editProduct };
